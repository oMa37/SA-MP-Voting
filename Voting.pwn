...
CMD:vote(playerid, params[])
{
	...
	Voting[Vote] = text;
	OnVote = 1;
	if(sscanf(params, "s[50]", text)) return SCM(playerid, red, "Make a vote: /vote <Text>");

	format(str, sizeof(str), "%s has started a vote: %s", GetName(playerid), text);
...
CMD:votes(playerid, params[])
{
	...
	foreach(new i : Player)
	{
		if(Voting[VoteY])
		{
			format(str, 128, "{FFFFFF}%s - {00FF00}Yes\n", GetName(i));
			strcat(string, str, sizeof(string));
			Players++;
		}
		if(Voting[VoteN])
		{
			format(str, 128, "{FFFFFF}%s - {FF0000}No\n", GetName(i));
			strcat(string, str, sizeof(string));
			Players++;
		}		
	}
...
{
    new string[1400];
    strcat(string, "{FFFFFF}Scripter: {00FF00}_oMa37\n");
    ShowPlayerDialog(playerid, 547, DIALOG_STYLE_MSGBOX, "Credits", string, "Close", "");
    return 1;
}
